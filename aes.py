# ----------------------------------------------------------------------------------
# Company: Procenne Digital Security
...
# Description: 256 Bitlik Aes sifreleme algoritmasinin python ile modellemesinin yapilmasi
...
#KeyExpansion function for Encryption 256------------------------------------------------------------------------
...
#AES256 Function ----------------------------------------------------------------
def aes(pt,key):
    Start_of_Round = pt #plain_text 128 bit
    Round_Key_Value = key #256 bit key
    key_round = 0  #Key expansion islemimin islem sayisini sayacak round degerim.
    Round_Key_Number = Key1(Round_Key_Value) #Baslangic olarak 256 bitlik Round_Key_Value mun ilk 128 bitini alıp isleme girmesini sagliyorum.
    ...
    print("round[",i,"].Output:",hex(ct))
    
#-------------------------Inputs---------------------------------------------

Plain_text = 0x00112233445566778899aabbccddeeff#sifrelenecek degerim(128 bit)
Key = 0x000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f#anahtar (256 bit)
